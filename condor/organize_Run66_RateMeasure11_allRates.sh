...
do
  ...
  cat /data/chocula/villaa/k100Sim_Data/captureCal/Run66ShieldRateMeasure11_PuBe_${DATASETIDHEX}_1M* |awk '{if($22==1 && $2==1){print $1"\t"$2"\t"$22}}' |awk -v rad=${rad} -v offset=${i} -v rateZIP=${rateZIP} -v rateNaIB=${rateNaIB} -v rateNaIC=${rateNaIC} -v num=${num} 'END{print rad"\t"NR"\t"rateZIP"\t"rateNaIB"\t"rateNaIC"\t"num}'
  let COUNTER++
done
